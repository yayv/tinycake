...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_smarty_ref(&$indexes)
    {
        ...
        switch ($_ref) {
            ...
            case 'get':
                $compiled_ref = ($this->request_use_auto_globals) ? '$_GET' : "\$CONFIG['HTTP_GET_VARS']";
                break;

            case 'post':
                $compiled_ref = ($this->request_use_auto_globals) ? '$_POST' : "\$CONFIG['HTTP_POST_VARS']";
                break;

            case 'cookies':
                $compiled_ref = ($this->request_use_auto_globals) ? '$_COOKIE' : "\$CONFIG['HTTP_COOKIE_VARS']";
                break;

            case 'env':
                $compiled_ref = ($this->request_use_auto_globals) ? '$_ENV' : "\$CONFIG['HTTP_ENV_VARS']";
                break;

            case 'server':
                $compiled_ref = ($this->request_use_auto_globals) ? '$_SERVER' : "\$CONFIG['HTTP_SERVER_VARS']";
                break;

            case 'session':
                $compiled_ref = ($this->request_use_auto_globals) ? '$_SESSION' : "\$CONFIG['HTTP_SESSION_VARS']";
                break;
